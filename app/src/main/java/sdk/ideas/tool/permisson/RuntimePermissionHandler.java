...
public class RuntimePermissionHandler extends BaseHandler
{
	...
	private ArrayList<PermissionParameter> permissionData = null;
	...
	{
		permissionData = new ArrayList<PermissionParameter>();

		for (int i = 0; i < permissionsArrayData.size(); i++)
		{
			permissionData
					.add(new PermissionParameter(permissionsArrayData.get(i), PermissionParameter.PERMISSION_DENIED));
		}
		...
		if (null != permissionData && permissionData.size() != 0)
		{
			for (int i = 0; i < permissionData.size(); i++)
			{
						...
						permissionData.get(i).permissionName) == PackageManager.PERMISSION_DENIED)
				{
						...
						{
							permissionData.get(i).permissionState = PermissionParameter.PERMISSION_NEVER_ASK_AGAIN;
						}
						else
						{
							permissionData.get(i).permissionState = PermissionParameter.PERMISSION_DENIED;
						}
					
				}
				else
				{
					permissionData.get(i).permissionState = PermissionParameter.PERMISSION_GRANT;
				}
			}
	...
	private void requestPermission()
	{
		...
		for (int i = 0; i < permissionData.size(); i++)
		{
			if (permissionData.get(i).permissionState != PermissionParameter.PERMISSION_GRANT)
			{
