...
public class WiFiConfigHandler
{
	...
	public WiFiConfigHandler(Context context)
	{
		...
		wifi_open_config = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
		connectivityManager = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
		if (!isConnected(wifi_open_config))
		{
	 ...
	 * 
	 * @params SSID, PASSWORD - WiFi SSID and PASSWORD should be passed in.
	 */
	...
	public void saveEAPConfig(String userName, String passString, String eapMethod, String phase2AuthMethod)
	{
		 ...
		 ****************************************************/
		String ENTERPRISE_EAP = "TTLS";// Should be one of -
										// TLS/TTLS/PEAP/PWD/SIM/AKA/FAST/LEAP
		...
		String ENTERPRISE_PHASE2 = "PAP";// Should be one of -
		...
		try
		{
			...
			Field wcefAnonymousId = null, wcefCaCert = null, wcefClientCert = null, wcefEap = null, wcefIdentity = null,
					wcefPassword = null, wcefPhase2 = null, wcefPrivateKey = null;
	...
	public void createLogFile()
	{
		...
		try
		{
			...
			Log.e("SD CARD WRITE ERROR : ", "SD CARD mounted and writable? " + root.canWrite());
			if (root.canWrite())
			{
		...
		for (int i = 0; i < configList.size(); i++)
		{
			if (configList.get(i).SSID.contentEquals("\"SSID_NAME\""))
			{
				...
				try
				{
					...
					out.write("<<<<<<<<<<WifiPreference>>>>>>>>>>>>" + "[LEAP]"
							+ config.allowedAuthAlgorithms.get(AuthAlgorithm.LEAP));
					Log.d("<<<<<<<<<<WifiPreference>>>>>>>>>>>>",
							"[OPEN]" + config.allowedAuthAlgorithms.get(AuthAlgorithm.OPEN));
					out.write("<<<<<<<<<<WifiPreference>>>>>>>>>>>>" + "[OPEN]"
							+ config.allowedAuthAlgorithms.get(AuthAlgorithm.OPEN));
					Log.d("<<<<<<<<<<WifiPreference>>>>>>>>>>>>",
							"[SHARED]" + config.allowedAuthAlgorithms.get(AuthAlgorithm.SHARED));
					out.write("<<<<<<<<<<WifiPreference>>>>>>>>>>>>" + "[SHARED]"
							+ config.allowedAuthAlgorithms.get(AuthAlgorithm.SHARED));
					Log.d("<<<<<<<<<<WifiPreference>>>>>>>>>>>>", "[GROUP CIPHERS]");
					...
					out.write("<<<<<<<<<<WifiPreference>>>>>>>>>>>>" + "[CCMP]"
							+ config.allowedPairwiseCiphers.get(PairwiseCipher.CCMP));
					Log.d("<<<<<<<<<<WifiPreference>>>>>>>>>>>>",
							"[NONE]" + config.allowedPairwiseCiphers.get(PairwiseCipher.NONE));
					out.write("<<<<<<<<<<WifiPreference>>>>>>>>>>>>" + "[NONE]"
							+ config.allowedPairwiseCiphers.get(PairwiseCipher.NONE));
					Log.d("<<<<<<<<<<WifiPreference>>>>>>>>>>>>",
							"[TKIP]" + config.allowedPairwiseCiphers.get(PairwiseCipher.TKIP));
					out.write("<<<<<<<<<<WifiPreference>>>>>>>>>>>>" + "[TKIP]"
							+ config.allowedPairwiseCiphers.get(PairwiseCipher.TKIP));
					Log.d("<<<<<<<<<<WifiPreference>>>>>>>>>>>>", "[Protocols]");
				...
				{
					Log.e("WRITE ERROR : ", "Failed to write Logs to ReadConfigLog.txt");
					Log.e("WRITE ERROR : ", "Please take logs using Logcat");
					Log.e("<<<<<<<<<<WifiPreference>>>>>>>>>>>>",
							"Could not write to ReadConfigLog.txt" + e.getMessage());
				}
				/* reflection magic */
				...
				try
				{
					...
					Field wcefAnonymousId = null, wcefCaCert = null, wcefClientCert = null, wcefEap = null,
							wcefIdentity = null, wcefPassword = null, wcefPhase2 = null, wcefPrivateKey = null;
					Field[] wcefFields = WifiConfiguration.class.getFields();
					// Dispatching Field vars
					for (Field wcefField : wcefFields)
					{
					...
					if (!noEnterpriseFieldType)
					{
						...
						Log.e("READ EROR : ", "Extract ReadConfigLog.txt from SD CARD");
					}
					else
					{
				...
				{
					Log.e("LOGGING EROR : ", "Failed to write Logs to ReadConfigLog.txt");
